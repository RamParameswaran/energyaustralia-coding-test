...
```
docker-compose exec log tail -f /var/log/energyaustralia-coding-test.log
```
...
around, but it needs to keep in mind. Generallt, you don not need to bring down this container.
  ...
  Most important logs are the one generated by Django, that are prepend with "energyaustralia-coding-test". A 
...
```
    ...
    docker run -it --name test -p 8080:80 --env-file=your_environment.env energyaustralia-coding-test
```
